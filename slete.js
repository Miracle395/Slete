...
try {
    ...
    if (!current || current.status !== 'active') {
        ...
        const replacement = allOracles
            .filter(o => o.status === 'active' && o.underlying_asset === baseAsset)
            .sort((a, b) => Number(a.activated_at || 0) - Number(b.activated_at || 0))[0];

        if (replacement?.oracle_id) {
            oracleId = replacement.oracle_id;
            this.state.activeDetailOracleId = oracleId;
            this.state.oracles = [...(this.state.oracles || []), replacement];
        }
    }
} catch (e) {
