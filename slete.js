...
async renderPortfolio() {
    ...
    try {
        ...
        const [summaryRes, pnlRes] = await Promise.all([
            fetch(`${PREDICT_SERVER}/managers/${addr}/positions/summary`),
            fetch(`${PREDICT_SERVER}/managers/${addr}/pnl?range=ALL`)
        ]);
        ...
        const totalEarned = typeof pnl === 'number'
            ? pnl
            : (pnl?.total_pnl ?? pnl?.pnl ?? 0);
        ...
        // positions list
        const positions = summary?.positions ?? summary?.data ?? [];
        ...
        positions.forEach(pos => {
            const oracle   = pos.oracle_id  ?? pos.oracle ?? '—';
            const strike   = pos.strike     ?? pos.price  ?? null;
            const status   = pos.status     ?? pos.state  ?? 'Pending';
            const decimals = strike >= 100 ? 2 : 4;

            const row = document.createElement('div');
            row.className = 'portfolio-row';
            row.innerHTML = `
                <span class="portfolio-row-pair">${oracle.slice(0,6)}..${oracle.slice(-4)}</span>
                <span class="portfolio-row-price">${strike !== null ? '$' + Number(strike).toFixed(decimals) : '—'}</span>
...
async renderLeaderboard() {
 ...
 if (!ranked.length) return; // empty state stays
  ...
  async initOracleData() {
    ...
    const SUI_WS = 'wss://rpc.testnet.sui.io:443';
   ...
   const seedPrice = async () => {
    ...
    // 2. Subscribe to live OraclePricesUpdated events via Sui WebSocket
    const subscribeToEvents = () => {
        const ws = new WebSocket(SUI_WS);
        ...
        ws.onopen = () => {
            ws.send(JSON.stringify({
                ...
                params: [{
                    MoveEventType: `${PREDICT_PKG}::oracle::OraclePricesUpdated`
                }]
            }));
        };
        ...
        ws.onmessage = (msg) => {
            try {
                ...
                const fields = parsed?.params?.result?.parsedJson;
                if (!fields) return;

                const oracleId = fields.oracle_id;
                const newPrice = Number(fields.spot) / 1_000_000_000;
                if (!oracleId || !newPrice) return;

                // find matching card(s)
                const cards = document.querySelectorAll('.pool-card');
                cards.forEach(card => {
                    const assetEl = card.querySelector('.asset');
                    const priceEl = card.querySelector('.live-price');
                    ...
                    if (ORACLE_IDS[asset] !== oracleId) return;
     ...
     const tickerEl = document.getElementById('tickerBTC');
if (tickerEl) {
    tickerEl.textContent = '$' + newPrice.toLocaleString('en-US', {
        minimumFractionDigits: 2, maximumFractionDigits: 2
    });
}
                    ...
                    if (detailPrice && document.getElementById('page-pool-detail')?.classList.contains('open')) {
                        ...
                        if (detailAsset === asset) {
                            ...
                            if (detailChange) {
                                detailChange.textContent = changeEl ? changeEl.textContent : '';
        ...
        // reconnect on drop (testnet WS can disconnect)
        ws.onclose = () => setTimeout(subscribeToEvents, 3000);
        ws.onerror = () => ws.close();
    };
...
subscribeToEvents();
    ...
    const strikeRaw = Math.round(parseFloat(val) * 1_000_000_000);
    ...
    try {
        const { Transaction } = await import('https://unpkg.com/@mysten/sui@latest/dist/index.js')
