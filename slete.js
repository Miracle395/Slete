...
try {
    ...
    // ── Step 2: Build the MarketKey and mint ─────────────────────────
    btn.textContent = 'Submitting on Sui...';

    // Get expiry from oracle data
    const oracleData = (this.state.oracles || []).find(o => o.oracle_id === oracleId);
    const expiryRaw  = Number(oracleData?.expiry || 0);
    // expiry must be in milliseconds (u64)
    const expiryMs   = expiryRaw > 1e12 ? expiryRaw : expiryRaw * 1000;

    const tx = new Transaction();
    tx.setSender(this.state.walletAddress);

    // Build the MarketKey inline using market_key::up() or down()
    // For this app we're always predicting a price — use 'up' direction (is_up = true)
    ...
    tx.moveCall({
        ...
        arguments: [
            tx.object(PREDICT_OBJECT),   // &mut Predict
            tx.object(managerObjId),      // &mut PredictManager
            tx.object(oracleId),          // &OracleSVI
            marketKey,                    // key: MarketKey
            tx.pure.u64(BigInt(1_000_000)), // 1 DUSDC (6 decimals)
