...
const active = oracles
    .filter(function (o) {
        ...
        const expiryRaw = Number(o.expiry || 0);
        const expiryMs = expiryRaw > 1e12 ? expiryRaw : expiryRaw * 1000;
        return expiryMs > now;
    })
    .sort(function (a, b) {
        return Number(a.expiry || 0) - Number(b.expiry || 0);
    });
...
async hydrateCardStats(oracles) {
    ...
    await Promise.all(oracles.map(async oracle => {
        ...
        try {
            ...
            // Filter to Slete-only wallets
            const sletePositions = positions; // show all positions for this oracle
    ...
    // Re-sort cards by prize pool descending
const grid = document.getElementById('active-pools-grid');
if (grid) {
    const cards = Array.from(grid.querySelectorAll('.pool-card'));
    cards.sort((a, b) => {
        const aVal = parseFloat(a.querySelector('.stat-prize')?.textContent) || 0;
        const bVal = parseFloat(b.querySelector('.stat-prize')?.textContent) || 0;
        return bVal - aVal;
    });
...
const tickSize = BigInt(liveState?.oracle?.tick_size || 1_000_000_000);
const strikeRaw = Number((BigInt(strikeRawUnsnapped) / tickSize) * tickSize);
console.log('[Slete DEBUG] strikeRaw (snapped):', strikeRaw, 'tick:', tickSize.toString());

// Pre-flight: check ask_bounds before sending tx
const askBounds = liveState?.oracle?.ask_bounds ?? liveState?.ask_bounds ?? null;
if (askBounds) {
    const lo = Number(askBounds.lower ?? askBounds.lo ?? askBounds.min ?? askBounds[0] ?? 0);
    const hi = Number(askBounds.upper ?? askBounds.hi ?? askBounds.max ?? askBounds[1] ?? Infinity);
