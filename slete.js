...
// Periodically recheck for oracles that have become mintable
// (ask_bounds published) since last load, so new pools appear
// without requiring a manual refresh.
if (!this._oracleRecheckInterval) {
    this._oracleRecheckInterval = setInterval(() => {
        this.initOracleData();
...
// Drop oracles that aren't mintable yet (ask_bounds not published).
// Without this, users hit assert_mintable_ask (abort code 7) on submit.
const boundsChecks = await Promise.all(
    active.map(o =>
        fetch(`https://predict-server.testnet.mystenlabs.com/oracles/${o.oracle_id}/ask-bounds`)
            .then(r => r.ok ? r.json() : null)
            .then(b => ({ oracle_id: o.oracle_id, mintable: b !== null && b !== undefined }))
            .catch(() => ({ oracle_id: o.oracle_id, mintable: false }))
    )
);

const mintableIds = new Set(
    boundsChecks.filter(b => b.mintable).map(b => b.oracle_id)
);

active = active.filter(o => mintableIds.has(o.oracle_id));
...
console.log('[Slete DEBUG] strikeRaw:', strikeRaw);
...
updateWinEstimate() {
    ...
    if (prizePool > 0) {
        const totalPool = prizePool + stake;
        winEl.textContent = '~' + totalPool.toFixed(2) + ' DUSDC';
    } else {
        // no prize pool data yet — show stake × rough multiplier hint
        winEl.textContent = '~' + (stake * 2).toFixed(0) + '+ DUSDC';
    }
},
