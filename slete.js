...
async initLabs() {
    ...
    this._labsTimer = setInterval(() => {
        ...
        if (minsEl) minsEl.textContent = h > 0 ? `${h}h ${String(m).padStart(2,'0')}` : String(m).padStart(2,'0');
  ...
  // Gate / pick / locked state
    ...
    const pickEl   = document.getElementById('labsPickGrid');
...
// Real Order Book volume = sum of all position costs for this oracle
...
if (obVolEl) {
 ...
 if (todayPick) {
    ...
    // Pick buttons
    const handlePick = (direction, evt) => {
        if (!this.state.walletConnected || !managerObjId) return;
        // Ripple effect
    ...
    window.addEventListener('slete:walletUpdate', (e) => {
        ...
        if (e.detail.connected && !todayPick) {
            if (gateEl)   gateEl.style.display  = 'none';
            if (pickEl)   pickEl.style.display   = 'block';
        }
    }, { once: true });
...
_updateLabsPayout(stake, upProb) {
    ...
    // Payout = stake / upProb (simplified binary option math)
    // Each card shows its own win estimate at the current odds
    [5, 25, 100].forEach(amt => {
        const payout = (amt / upProb).toFixed(2);
        const el = document.getElementById('labsWin' + amt);
        if (el) el.textContent = '$' + payout;
    });
},
...
_drawLabsSparkline() {
    ...
    ctx.textAlign = 'left';
    const labels = [maxP, (maxP + minP) / 2, minP];
    labels.forEach(lp => {
        ctx.fillText('$' + lp.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 }), w - pad.right + 6, yOf(lp) + 4);
    });
},
