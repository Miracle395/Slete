...
async initLabs() {
    const stored = JSON.parse(localStorage.getItem('slete_streak') || '{}');
    const streak = stored.streak || 0;
    const todayKey = new Date().toISOString().slice(0, 10);

    // todayPick is now chain-verified below — localStorage only holds streak count
    let todayPick = null;

    // Streak display — both hero and locked panel
    const streakNumEl  = document.getElementById('labsStreakNum');
    const streakHeroEl = document.getElementById('labsStreakHeroCount');
    if (streakHeroEl) streakHeroEl.textContent = streak;

    // Market sub header date
    const subEl = document.getElementById('labsMarketSub');
    if (subEl) {
        const now = new Date();
        const dateStr = now.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        subEl.textContent = `${dateStr} · Closes midnight UTC`;
    }

    // Price to Beat = entry price stored from yesterday's pick, or live price
    const ptbEl = document.getElementById('labsPriceToBeat');

    // Sparkline state
    this._labsPriceHistory = this._labsPriceHistory || [];
    this._labsSelectedStake = this._labsSelectedStake || 5;
    
    const removeSkel = (id) => {
    const el = document.getElementById(id);
    if (el) el.classList.remove('skel','skel-sm','skel-md','skel-lg','skel-pct','skel-bar');
};

    // Countdown timer
    if (this._labsTimer) clearInterval(this._labsTimer);
    this._labsTimer = setInterval(() => {
        const now = new Date();
        ...
        const minsEl = document.getElementById('labsHeaderMins');
        const secsEl = document.getElementById('labsHeaderSecs');
        const lockedTimerEl = document.getElementById('labsLockedTimer');
...
const clockEl = document.getElementById('labsTimerClock');
if (clockEl) { removeSkel('labsTimerClock'); clockEl.textContent = `${hh}:${mm}:${ss}`; }
        if (lockedTimerEl) lockedTimerEl.textContent = `${h > 0 ? h + 'h ' : ''}${String(m).padStart(2,'0')}m ${String(secs).padStart(2,'0')}s`;
    }, 1000);

    // Fetch live BTC price from DeepBook oracle
    const btcOracle = (this.state.oracles || []).find(o =>
        (o.underlying_asset || '').toUpperCase() === 'BTC'
    );
    const storedOracleIds = JSON.parse(localStorage.getItem('slete_oracle_ids') || '[]');
    const btcOracleId = btcOracle?.oracle_id || storedOracleIds[0];

    const updatePriceDisplay = (price) => {
        this._labsBtcPrice = price;
        this._labsPriceHistory.push({ t: Date.now(), p: price });
        if (this._labsPriceHistory.length > 60) this._labsPriceHistory.shift();

        const priceEl = document.getElementById('labsDailyPrice');
        if (priceEl) {
    removeSkel('labsDailyPrice');
    const prevPrice = this._labsLastDisplayPrice || price;
    priceEl.textContent = '$' + price.toLocaleString('en-US', {
                minimumFractionDigits: 2, maximumFractionDigits: 2
            });
            if (price !== prevPrice) {
                ...
                priceEl.classList.add(price > prevPrice ? 'tick-up' : 'tick-down');
            }
            this._labsLastDisplayPrice = price;
        }

        // Price to Beat = stored entry price from when the pick was made.
        // Only set once per session and never from the same tick as current price.
        if (!this._labsPriceToBeat) {
            if (stored.entryPrice && stored.entryPrice > 0) {
                this._labsPriceToBeat = stored.entryPrice;
            } else if (this._labsPriceHistory.length > 1) {
                // Use the first price we saw this session, not the current one
                this._labsPriceToBeat = this._labsPriceHistory[0].p;
            }
            // If we only have 1 data point, leave it unset — delta stays blank
        }
 if (ptbEl) ptbEl.textContent = '$' + this._labsPriceToBeat.toLocaleString('en-US', {
            minimumFractionDigits: 2, maximumFractionDigits: 2
        });

        // Delta pill
        const delta = price - this._labsPriceToBeat;
        const deltaEl = document.getElementById('labsPriceDeltaPill');
        if (deltaEl) {
            const sign = delta >= 0 ? '▲ $' : '▼ $';
            deltaEl.textContent = sign + Math.abs(delta).toFixed(0);
            deltaEl.style.background = delta >= 0 ? 'var(--green-soft)' : 'var(--red-soft)';
            deltaEl.style.color = delta >= 0 ? 'var(--green)' : 'var(--red)';
        }

       // Price current value color
        const curVal = document.getElementById('labsDailyPrice');
        if (curVal) curVal.style.color = delta >= 0 ? 'var(--green)' : 'var(--red)';

        // VS card — side tinting
        const currentSide = document.getElementById('labsVersusCurrentSide');
        if (currentSide) {
            currentSide.classList.toggle('labs-versus-side--winning', delta >= 0);
            currentSide.classList.toggle('labs-versus-side--losing', delta < 0);
        }

        // VS card — progress bar
        // Bar position = how far current is from ptb, clamped 10–90%
        const barEl = document.getElementById('labsVersusBar');
        if (barEl && this._labsPriceToBeat) {
            const range = this._labsPriceToBeat * 0.005; // ±0.5% window
            const clamped = Math.max(-range, Math.min(range, delta));
            const pct = 50 + (clamped / range) * 40; // 10–90%
            barEl.style.width = pct.toFixed(1) + '%';
            barEl.style.background = delta >= 0 ? 'var(--green)' : 'var(--red)';
        }

        // Draw sparkline
        this._drawLabsSparkline();
    };

    if (btcOracleId) {
        const fetchPrice = () => {
            fetch(`https://predict-server.testnet.mystenlabs.com/oracles/${btcOracleId}/prices/latest`)
                .then(r => r.json())
                .then(data => {
                    if (!data?.spot) return;
                    updatePriceDisplay(data.spot / 1_000_000_000);
                })
                .catch(() => {
                    const el = document.getElementById('labsDailyPrice');
                    if (el && el.textContent === '—') el.textContent = 'Unavailable';
                });
        };
        fetchPrice();
        if (this._labsPricePoll) clearInterval(this._labsPricePoll);
        this._labsPricePoll = setInterval(fetchPrice, 10000);
    }
    
  // Apply skeletons to all blank labs elements
const skelTargets = {
    labsTimerClock:  'skel skel-md',
    labsBullPct:     'skel skel-pct',
    labsBearPct:     'skel skel-pct',
    labsPriceToBeat: 'skel skel-lg',
    labsDailyPrice:  'skel skel-lg',
    labsTrendVal:    'skel skel-sm',
    labsObTraders:   'skel skel-sm',
    labsObVol:       'skel skel-sm',
};
Object.entries(skelTargets).forEach(([id, cls]) => {
    const el = document.getElementById(id);
    if (el && el.textContent.trim() === '—') {
        el.dataset.skelOrigText = el.textContent;
        el.textContent = '';
        el.classList.add(...cls.split(' '));
    }
});
  
  // Gate / pick / locked state
const gateEl   = document.getElementById('labsGate');
    const pickEl   = document.getElementById('labsPickUp')?.closest('.labs-pick-buttons');
    const lockedEl = document.getElementById('labsLocked');

   // Fetch real positions to calculate odds
const PREDICT_SERVER_BASE = 'https://predict-server.testnet.mystenlabs.com';
  
const positionsRes = await fetch(`${PREDICT_SERVER_BASE}/positions/minted?oracle_id=${btcOracleId}`);
const positionsData = positionsRes.ok ? await positionsRes.json() : null;
const positions = positionsData ? (Array.isArray(positionsData) ? positionsData : (positionsData.positions || positionsData.data || [])) : [];

if (positions.length > 0) {
    // Count UP vs DOWN positions by quantity
    const upQty = positions.filter(p => p.is_up === true).reduce((sum, p) => sum + (Number(p.quantity) || 0), 0);
    const downQty = positions.filter(p => p.is_up === false).reduce((sum, p) => sum + (Number(p.quantity) || 0), 0);
    const totalQty = upQty + downQty || 1;
    const upProb = upQty / totalQty;
    const downProb = downQty / totalQty;
    
    console.log('[Slete Labs] Real odds — UP:', (upProb * 100).toFixed(1) + '%', 'DOWN:', (downProb * 100).toFixed(1) + '%');
    
    // Only update odds display if we're in the pick panel (not locked)
    if (pickEl && pickEl.style.display !== 'none') {
        const upOddsEl = document.getElementById('labsOddsUp');
        const downOddsEl = document.getElementById('labsOddsDown');
        if (upOddsEl) upOddsEl.textContent = Math.round(upProb * 100) + '¢';
        if (downOddsEl) downOddsEl.textContent = Math.round(downProb * 100) + '¢';
        
        // Payout based on stake
        this._updateLabsPayout(this._labsSelectedStake, upProb);
    }
} else {
    console.log('[Slete Labs] No positions found, skipping real odds');
}

// Order book toggle
const obRow  = document.getElementById('labsObRow');
const obBody = document.getElementById('labsObBody');
if (obRow && obBody) {
    obRow.addEventListener('click', () => {
        const isOpen = obRow.classList.toggle('open');
        obBody.style.display = isOpen ? 'block' : 'none';
    });
}

// Populate OB side volumes
const upCost   = positions.filter(p => p.is_up === true).reduce((s, p) => s + (Number(p.cost) || 0), 0) / 1e6;
const downCost = positions.filter(p => p.is_up === false).reduce((s, p) => s + (Number(p.cost) || 0), 0) / 1e6;
const fmt = v => v >= 1000 ? '$' + (v/1000).toFixed(1) + 'K' : '$' + v.toFixed(0);
const upVolEl     = document.getElementById('labsObUpVol');
const downVolEl   = document.getElementById('labsObDownVol');
const obVolEl     = document.getElementById('labsObVol');
const obTradersEl = document.getElementById('labsObTraders');
if (upVolEl)   upVolEl.textContent   = fmt(upCost);
if (downVolEl) downVolEl.textContent = fmt(downCost);
if (obVolEl) {
    removeSkel('labsObVol');
    const totalVol = positions.reduce((sum, p) => sum + (Number(p.cost) || 0), 0) / 1e6;
    obVolEl.textContent = totalVol >= 1000
        ? '$' + (totalVol / 1000).toFixed(1) + 'K Vol.'
        : '$' + totalVol.toFixed(0) + ' Vol.';
}

const uniqueTraders = new Set(positions.map(p => p.manager_id)).size;
if (obTradersEl) { removeSkel('labsObTraders'); obTradersEl.textContent = uniqueTraders.toLocaleString() + ' Traders'; }

// Market header stats strip
const totalVol = upCost + downCost;
const bullishPct = positions.length > 0
    ? Math.round((positions.filter(p => p.is_up === true).reduce((s, p) => s + (Number(p.quantity) || 0), 0) /
      (positions.reduce((s, p) => s + (Number(p.quantity) || 0), 0) || 1)) * 100)
    : null;

const statTradersEl = document.getElementById('labsStatTraders');
const statVolEl     = document.getElementById('labsStatVol');
const statBullEl    = document.getElementById('labsStatBullish');

if (statTradersEl) statTradersEl.textContent = uniqueTraders.toLocaleString() + ' Traders';
if (statVolEl)     statVolEl.textContent     = fmt(totalVol) + ' Vol.';
if (statBullEl)    statBullEl.textContent    = bullishPct !== null ? bullishPct + '% Bullish' : '— Bullish';

const bullPctEl = document.getElementById('labsBullPct');
const bearPctEl = document.getElementById('labsBearPct');
const bullBarEl = document.getElementById('labsBullBar');
const bearBarEl = document.getElementById('labsBearBar');
if (bullishPct !== null) {
    const bearPct = 100 - bullishPct;
    removeSkel('labsBullPct'); removeSkel('labsBearPct');
    if (bullPctEl) bullPctEl.textContent = bullishPct + '%';
    if (bearPctEl) bearPctEl.textContent = bearPct + '%';
    setTimeout(() => {
        if (bullBarEl) bullBarEl.style.width = bullishPct + '%';
        if (bearBarEl) bearBarEl.style.width = bearPct + '%';
    }, 400);
}

    // Chain-verify: does this wallet have a position on today's BTC oracle?
    const managerObjId = this.state.managerObjectId
        || localStorage.getItem(`slete_manager_${this.state.walletAddress}`)
        || null;

    if (this.state.walletConnected && managerObjId && btcOracleId) {
        try {
            const myPositions = positions.filter(p =>
                p.manager_id === managerObjId &&
                p.oracle_id  === btcOracleId
            );

            if (myPositions.length > 0) {
                // Use the most recent position's is_up field as direction
                const latest   = myPositions[myPositions.length - 1];
                todayPick       = latest.is_up ? 'up' : 'down';

                // Persist streak to localStorage (count only, not gate)
                const saveData = {
                    streak: streak,
                    best:   stored.best || 0,
                    date:   todayKey,
                    pick:   todayPick,
                    entryPrice: this._labsBtcPrice || null,
                };
                localStorage.setItem('slete_streak', JSON.stringify(saveData));
            }
        } catch (e) {
            console.warn('[Slete Labs] Chain position check failed:', e);
        }
    }
 ...
 if (todayPick) {
        ...
        if (lockedEl) {
            ...
            const badge = document.getElementById('labsLockedBadge');
            if (badge) {
                badge.textContent = todayPick.toUpperCase();
                badge.className   = 'labs-locked-badge ' + todayPick;
                // Animate in
                requestAnimationFrame(() => {
                    lockedEl.classList.add('animate-in');
                    setTimeout(() => badge.classList.add('pop'), 120);
                    // Streak count-up
                    const streakEl = document.getElementById('labsStreakNum');
                    if (streakEl && streak > 0) {
                        ...
                        const step = () => {
                            ...
                            streakEl.classList.remove('labs-streak-num-flash');
                            void streakEl.offsetWidth;
                            streakEl.classList.add('labs-streak-num-flash');
                            if (count < target) setTimeout(step, 120);
                        };
                        setTimeout(step, 300);
                    }
                });
        ...
        // Show momentum bar with real UP% from positions
        const momentumWrap = document.getElementById('labsMomentumWrap');
        const momentumFill = document.getElementById('labsMomentumFill');
        if (momentumWrap && momentumFill && positions.length > 0) {
            momentumWrap.style.display = 'block';
            const upQty   = positions.filter(p => p.is_up === true).reduce((s, p) => s + (Number(p.quantity) || 0), 0);
            const downQty = positions.filter(p => p.is_up === false).reduce((s, p) => s + (Number(p.quantity) || 0), 0);
            const upPct   = (upQty / (upQty + downQty || 1)) * 100;
            setTimeout(() => { momentumFill.style.width = upPct + '%'; }, 400);
        }
    ...
    // Stake cards
    document.querySelectorAll('.labs-stake-card').forEach(card => {
        card.addEventListener('click', () => {
            document.querySelectorAll('.labs-stake-card').forEach(c => c.classList.remove('active'));
            card.classList.add('active');
            this._labsSelectedStake = Number(card.dataset.stake);
            const upProb = this._labsLastUpProb || 0.65;
            this._updateLabsPayout(this._labsSelectedStake, upProb);
        });
    });

// TradingView chart helper — reads current theme, strips all toolbar clutter
const tvIntervalMap = { '5m': '5', '15m': '15', '1h': '60', '4h': '240' };
this._setLabsTV = (interval) => {
    const isDark = document.body.classList.contains('dark');
    const theme = isDark ? 'dark' : 'light';
    // background param must match our surface color exactly
    const bg = isDark ? '%231C1611' : '%23FCFAF7';
    const iframe = document.getElementById('labsTVChart');
    if (!iframe) return;
    iframe.src = `https://s.tradingview.com/widgetembed/?frameElementId=labsTVChart`
        + `&symbol=BINANCE%3ABTCUSDT&interval=${interval}`
        + `&hidesidetoolbar=1&hidetoptoolbar=1`
        + `&symboledit=0&saveimage=0`
        + `&studies=%5B%5D`
        + `&theme=${theme}`
        + `&style=3`
        + `&timezone=UTC&locale=en`
        + `&hide_legend=1&hide_volume=1`
        + `&background=${bg}`
        + `&gridColor=rgba(0%2C0%2C0%2C0.04)`;
};

// Set on init
const initInterval = document.querySelector('.labs-scrub-chip.active')?.dataset.window || '5m';
this._setLabsTV(tvIntervalMap[initInterval] || '5');

// Scrubber chips
document.querySelectorAll('.labs-scrub-chip').forEach(chip => {
    chip.addEventListener('click', () => {
        document.querySelectorAll('.labs-scrub-chip').forEach(c => c.classList.remove('active'));
        chip.classList.add('active');
        this._setLabsTV(tvIntervalMap[chip.dataset.window] || '5');
    });
});

    // Gate connect
    const gateConnectBtn = document.getElementById('labsGateConnect');
    if (gateConnectBtn) {
        gateConnectBtn.onclick = () => {
            if (typeof window.handleConnect === 'function') {
                window.handleConnect(document.getElementById('walletBtn'));
            }
        };
    }

    // Pick buttons
const handlePick = async (direction, evt) => {
    if (!this.state.walletConnected || !managerObjId) {
        if (gateEl) {
            gateEl.style.display = 'flex';
            const txt = gateEl.querySelector('.labs-gate-sheet-text');
            if (txt) txt.textContent = 'Connect wallet to place prediction';
            const btn = document.getElementById('labsGateConnect');
            if (btn) btn.textContent = 'Connect Wallet';
        }
        return;
    }

    this._labsSelectedDirection = direction;
    this._updateLabsPayout(this._labsSelectedStake || 5, this._labsLastUpProb || 0.65);

    // Ripple
    const btn = direction === 'up'
        ? document.getElementById('labsPickUp')
        : document.getElementById('labsPickDown');
    const ripple = btn?.querySelector('.labs-pick-ripple');
    if (ripple) {
    ...
    try {
        const Transaction = window.__SuiTransaction;
        if (!Transaction || !btcOracleId) throw new Error('SETUP_ERROR');
        if (typeof window.signAndExecuteTransaction !== 'function') throw new Error('WALLET_ERROR');

        if (btn) btn.textContent = direction === 'up' ? 'Placing Up...' : 'Placing Down...';

        // Get fresh oracle state for expiry + tick size
        const liveStateRes = await fetch(`https://predict-server.testnet.mystenlabs.com/oracles/${btcOracleId}/state`);
        const liveState = liveStateRes.ok ? await liveStateRes.json() : null;
        const tickSize  = Number(liveState?.oracle?.tick_size || 1_000_000_000);

        // Strike = current BTC price snapped to tick
        const strikeRawUnsnapped = Math.round((this._labsBtcPrice || 0) * 1_000_000_000);
        const strikeRaw = Math.round(strikeRawUnsnapped / tickSize) * tickSize;

        const oracleData = (this.state.oracles || []).find(o => o.oracle_id === btcOracleId);
        const expiryRaw  = Number(oracleData?.expiry || liveState?.oracle?.expiry || 0);
        const expiryMs   = expiryRaw > 1e12 ? expiryRaw : expiryRaw * 1000;

        // Stake amount in DUSDC (6 decimals)
        const stake = this._labsSelectedStake || 5;
        const DEPOSIT_AMOUNT = BigInt(Math.round(stake * 1_000_000));

        // Fetch user's DUSDC coin
        const coinsRes = await fetch('https://sui-testnet.gateway.tatum.io', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-api-key': 't-6a1314026dcffd29f3321133-b2b7fb9669494fdebadaf640' },
            body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'suix_getCoins', params: [this.state.walletAddress, DUSDC_TYPE, null, 1] })
        });
        const coinsData = await coinsRes.json();
        const dusdcCoin = coinsData?.result?.data?.[0];
        if (!dusdcCoin) throw new Error('NO_DUSDC_BALANCE');
        ...
        // Build MarketKey — up or down
        const marketKey = tx.moveCall({
            target: `${PREDICT_PKG}::market_key::${direction}`,
            arguments: [
                tx.pure.id(btcOracleId),
                tx.pure.u64(BigInt(expiryMs)),
                tx.pure.u64(BigInt(strikeRaw)),
            ],
        });
        ...
        const digest = await window.signAndExecuteTransaction(tx);
        console.log('[Slete Labs] Position minted:', digest);
        
         // Save streak locally first
    const saveData = {
    ...
    if (lockedEl) {
        ...
        if (badge) {
            ...
            badge.className   = 'labs-locked-badge ' + direction;
        }
    }
        
        // Hide pick UI, show locked state
const pickButtons = document.querySelector('.labs-pick-buttons');
const stakeCards  = document.getElementById('labsStakeCards');
if (pickButtons) pickButtons.style.display = 'none';
if (stakeCards)  stakeCards.style.display  = 'none';
const bottomBar = document.getElementById('labsBottomBar');
if (bottomBar) bottomBar.style.display = 'none';
        
        if (btn) btn.textContent = direction === 'up' ? 'Up ✓' : 'Down ✓';

    } catch (err) {
        console.error('[Slete Labs] Mint failed:', err);
        if (btn) btn.textContent = err.message === 'NO_DUSDC_BALANCE'
            ? 'No DUSDC balance'
            : 'Failed — try again';
        setTimeout(() => {
            if (btn) btn.textContent = direction === 'up' ? 'Up' : 'Down';
    ...
    const upBtn   = document.getElementById('labsPickUp');
    const downBtn = document.getElementById('labsPickDown');
    if (upBtn)   upBtn.onclick   = (e) => handlePick('up', e);
    if (downBtn) downBtn.onclick = (e) => handlePick('down', e);

   // Wallet update listener — hide gate on connect
window.addEventListener('slete:walletUpdate', (e) => {
    if (document.getElementById('page-labs')?.style.display === 'none') return;
    if (e.detail.connected && gateEl) {
        gateEl.style.display = 'none';
        document.dispatchEvent(new CustomEvent('sleteWalletConnected'));
        if (pickEl) pickEl.style.display = 'flex';
        const stakeCards = document.getElementById('labsStakeCards');
        if (stakeCards) stakeCards.style.display = 'flex';
    }
}, { once: true });
},

_updateLabsPayout(stake, upProb) {
    this._labsLastUpProb = upProb;
    const direction = this._labsSelectedDirection || 'up';
    const prob = direction === 'up' ? upProb : (1 - upProb);
    [5, 25, 100].forEach(amt => {
        const payout = amt / prob;
        const el = document.getElementById('labsWin' + amt);
        if (el) el.textContent = '$' + payout.toFixed(2);
    });
},

_drawLabsSparkline() {
    const canvas = document.getElementById('labsSparkline');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const history = this._labsPriceHistory || [];
    ...
    for (let i = 0; i <= gridCount; i++) {
        const gy = pad.top + (i / gridCount) * (h - pad.top - pad.bottom);
        ctx.beginPath();
    ...
    // Vertical time markers (5 evenly spaced)
    const vCount = 4;
    ctx.strokeStyle = 'rgba(0,0,0,0.04)';
    for (let i = 1; i < vCount; i++) {
        const gx = pad.left + (i / vCount) * (w - pad.left - pad.right);
        ctx.beginPath();
    ...
    const pad = { top: 12, bottom: 20, left: 8, right: 60 };

    const xOf = (i) => pad.left + (i / (prices.length - 1)) * (w - pad.left - pad.right);
    const yOf = (p) => pad.top + (1 - (p - minP) / range) * (h - pad.top - pad.bottom);

  // Update trend label above chart
    const trendEl = document.getElementById('labsTrendVal');
    const trendPeriodEl = document.getElementById('labsTrendPeriod');
    if (trendEl && prices.length >= 2) {
        ...
        // Period label matches active scrubber window
        const activeChip = document.querySelector('.labs-scrub-chip.active');
        if (trendPeriodEl && activeChip) trendPeriodEl.textContent = activeChip.dataset.window;
    }

    // Target line (price to beat)
    const ptb = this._labsPriceToBeat;
    if (ptb && ptb >= minP && ptb <= maxP) {
        const ty = yOf(ptb);
        ctx.beginPath();
        ctx.setLineDash([5, 4]);
        ctx.strokeStyle = 'rgba(200,145,79,0.55)';
        ctx.lineWidth = 1.2;
        ctx.moveTo(pad.left, ty);
        ctx.lineTo(w - pad.right + 10, ty);
        ctx.stroke();
        ctx.setLineDash([]);
    }
    ...
    const lineColor = lastPrice >= (ptb || lastPrice) ? '#2EA878' : '#D95F4B';
    ...
    // Place/move the live DOM dot overlay for CSS pulse animation
    const chartWrap = document.querySelector('.labs-chart-wrap');
    if (chartWrap) {
        let liveDot = document.getElementById('labsLiveDot');
        if (!liveDot) {
            liveDot = document.createElement('div');
            liveDot.id = 'labsLiveDot';
            liveDot.className = 'labs-live-dot';
            chartWrap.style.position = 'relative';
            chartWrap.appendChild(liveDot);
        }
        const canvasEl = document.getElementById('labsSparkline');
        const canvasRect = canvasEl ? canvasEl.getBoundingClientRect() : null;
        const wrapRect  = chartWrap.getBoundingClientRect();
        if (canvasRect && wrapRect) {
            const scaleX = canvasRect.width  / (w * dpr);
            const scaleY = canvasRect.height / (h * dpr);
            liveDot.style.left = (canvasRect.left - wrapRect.left + lx * scaleX * dpr) + 'px';
            liveDot.style.top  = (canvasRect.top  - wrapRect.top  + ly * scaleY * dpr) + 'px';
        }
    }

    // Price axis — 4 evenly-spaced ticks with tick marks
    ctx.font = '600 10px IBM Plex Mono, monospace';
    ctx.textAlign = 'left';
    const tickCount = 4;
    for (let i = 0; i <= tickCount; i++) {
        const tickP = minP + (i / tickCount) * range;
        const ty = yOf(tickP);
        // Tick mark
        ctx.beginPath();
        ctx.strokeStyle = 'rgba(0,0,0,0.10)';
        ctx.lineWidth = 1;
        ctx.moveTo(w - pad.right, ty);
        ctx.lineTo(w - pad.right + 4, ty);
        ctx.stroke();
        // Label
        ctx.fillStyle = 'rgba(107,98,89,0.7)';
        const formatted = tickP >= 1000
            ? '$' + tickP.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
            : '$' + tickP.toFixed(2);
        ctx.fillText(formatted, w - pad.right + 7, ty + 4);
    }
},
