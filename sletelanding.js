...
const SlетeLanding = {
    ...
    initNavScroll() {
        ...
        // Inject BTC price badge into nav
        const badge = document.createElement('div');
        badge.id = 'navBtcBadge';
        badge.style.cssText = `
            display: inline-flex;
            align-items: center;
            gap: 6px;
        ...
        `;
        badge.innerHTML = `<span style="font-size:10px;color:var(--accent)">●</span><span id="navBtcPrice">BTC —</span>`;
        nav.querySelector('.nav-left').appendChild(badge);
        ...
        window.addEventListener('scroll', () => {
            ...
            nav.style.boxShadow = scrolled
                ? '0 2px 24px rgba(43,36,28,0.08)'
                : '';

            badge.style.opacity   = scrolled ? '1' : '0';
            badge.style.transform = scrolled ? 'translateY(0)' : 'translateY(-4px)';
        }, {passive: true});

        // Update nav BTC price alongside ticker
        const updateNavBtc = () => {
            const tickBtc = document.getElementById('tickBTC');
            const navBtcEl = document.getElementById('navBtcPrice');
            if (tickBtc && navBtcEl && tickBtc.textContent !== '$—') {
                navBtcEl.textContent = 'BTC ' + tickBtc.textContent;
            }
        };
        setInterval(updateNavBtc, 31_000);
        setTimeout(updateNavBtc, 3000);
    },
