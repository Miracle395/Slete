...
(function () {
  ...
  function handlePick(direction) {
    const walletAddr = window.__sleteWallet?.address;
  ...
  async function submitPrediction(direction) {
    try {
      ...
      const wallet = window.__sleteWallet;
  ...
  // Hook into whatever event slete.js fires when wallet connects
  document.addEventListener('sleteWalletConnected', () => {
  ...
  document.addEventListener('labsPageShown', () => {
    const addr = window.__sleteWallet?.address;
