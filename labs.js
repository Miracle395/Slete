...
(function () {
     ...
     // Pull position summary for vol/traders
      ...
      if (summaryRes.ok) {
        ...
        if (all.length > 0) {
          ...
          const vol = p => Number(p.cost || p.total_cost || 0);

          if (hasIsUp) {
            upVol   = all.filter(p => p.is_up === true).reduce((s, p)  => s + vol(p), 0) / 1e6;
            downVol = all.filter(p => p.is_up === false).reduce((s, p) => s + vol(p), 0) / 1e6;
          } else if (spotForSplit) {
            upVol   = all.filter(p => Number(p.strike || 0) >= spotForSplit).reduce((s, p) => s + vol(p), 0) / 1e6;
            downVol = all.filter(p => Number(p.strike || 0) <  spotForSplit).reduce((s, p) => s + vol(p), 0) / 1e6;
          }
 ...
 ;
  ...
  // Chain onto slete.js's onWalletConnected rather than overwriting it
