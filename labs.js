...
(function () {
  ...
  async function fetchLabsData() {
    try {
     ...
     // ── SVI implied probability (replaces stale position averaging) ──
      // Uses the oracle's live SVI vol surface to compute the binary UP
      // probability as N(d2) — the same math the contract runs at mint time.
      const stateRes = await fetch(
        `${PREDICT_SVR}/oracles/${oracle.oracle_id}/state`,
        { signal: AbortSignal.timeout(4000) }
      );
      if (stateRes.ok) {
        const sv = await stateRes.json();
        ...
        const spot    = Number(sv.latest_price.spot)    / 1e9;
        const forward = Number(sv.latest_price.forward) / 1e9;
        const expiry  = Number(sv.oracle.expiry);
        const T       = Math.max(1 / 365, (expiry - Date.now()) / (365.25 * 24 * 3600 * 1000));
        const k       = Math.log(forward / spot);

        const SCALE = 1e9;
        const a     =  Number(sv.latest_svi.a)     / SCALE;
        const b     =  Number(sv.latest_svi.b)     / SCALE;
        const rho   = (sv.latest_svi.rho_negative ? -1 : 1) * Number(sv.latest_svi.rho) / SCALE;
        const m     = (sv.latest_svi.m_negative   ? -1 : 1) * Number(sv.latest_svi.m)   / SCALE;
        const sigma =  Number(sv.latest_svi.sigma) / SCALE;

        const km  = k - m;
        const w   = a + b * (rho * km + Math.sqrt(km * km + sigma * sigma));
        const vol = Math.sqrt(Math.max(0, w) / T);
        const d2  = (-k / vol - vol * Math.sqrt(T) / 2);

        const bullProb = Ncdf(d2) * 100;
        state.upVol   = bullProb;
        state.downVol = 100 - bullProb;
      }

      // Trader count from positions (count only, no probability from here)
      ...
      if (summaryRes.ok) {
        ...
        state.traders = uniqueManagers.size || all.length;
      }
  ...
  function renderPoolData() {
    // upVol/downVol are now direct percentages from SVI (sum = 100)
    const bullPct = Math.round(state.upVol);
  ...
  function updateTVChart(window) {
    ...
    const intervalMap = { '1m': '1', '5m': '5', '1D': 'D' };
    const interval = intervalMap[window] || '5';
    els.tvChart.src =
      `https://s.tradingview.com/widgetembed/?frameElementId=labsTVChart&symbol=BINANCE%3ABTCUSDT&interval=${interval}&hidesidetoolbar=1&hidetoptoolbar=1&symboledit=0&saveimage=0&toolbarbg=F1F3F6&studies=[]&theme=light&style=1&timezone=Etc%2FUTC&withdateranges=0&showpopupbutton=0&allow_symbol_change=0&locale=en&no_referral_id=1&hide_legend=0&hide_volume=1`;
  }
  ...
  function lockCall(direction) {
    ...
    // Show share card
    showShareCard(direction);
  }
  
  
  // ── Share card ─────────────────────────────────────────────
function showShareCard(direction) {
  const overlay = document.getElementById('labsShareOverlay');
  if (!overlay) return;

  // Fill direction
  const dirEl   = document.getElementById('labsShareDirection');
  const arrowEl = document.getElementById('labsShareArrow');
  const labelEl = document.getElementById('labsShareDirLabel');
  if (dirEl)   { dirEl.className = 'labs-share-direction ' + direction; }
  if (arrowEl) arrowEl.textContent = direction === 'up' ? '↑' : '↓';
  if (labelEl) labelEl.textContent = direction.toUpperCase();

  // Fill prices
  const fmt = p => p ? '$' + p.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—';
  const ptbEl  = document.getElementById('labsSharePtb');
  const curEl  = document.getElementById('labsShareCurrent');
  if (ptbEl) ptbEl.textContent = fmt(state.priceToBeat);
  if (curEl) curEl.textContent = fmt(state.btcPrice);

  // Date
  const dateEl = document.getElementById('labsShareDate');
  if (dateEl) {
    const now = new Date();
  ...
  // Dismiss button
  const dismiss = document.getElementById('labsShareDismiss');
  if (dismiss) {
    dismiss.onclick = () => overlay.classList.remove('open');
  }
  ...
  // Copy link
  const copyBtn = document.getElementById('labsShareCopy');
  if (copyBtn) {
    copyBtn.onclick = () => {
      navigator.clipboard.writeText('https://slete.vercel.app/product.html#labs')
        .then(() => {
  ...
  // Post on X
  const xBtn = document.getElementById('labsShareX');
  if (xBtn) {
    xBtn.onclick = () => {
      const dirWord = direction === 'up' ? '📈 UP' : '📉 DOWN';
      const ptb = fmt(state.priceToBeat);
      const text = `I just called BTC ${dirWord} on @sleteapp\n\nPrice to beat: ${ptb}\nSettles midnight UTC\n\nslete.vercel.app`;
  ...
  // ── Hide labs chrome when navigating away from labs ────────
  function setLabsChrome(visible) {
    const bottomBar = document.getElementById('labsBottomBar');
    const gateSheet = document.getElementById('labsGate');
    if (!visible) {
      if (bottomBar) bottomBar.style.display = 'none';
      if (gateSheet) gateSheet.style.display = 'none';
    } else {
      // let normal wallet/locked logic decide what to show
      const addr = window.STATE?.walletAddr;
      if (!addr) {
        if (gateSheet) gateSheet.style.display = 'flex';
      } else if (!state.lockedCall) {
  ...
  document.addEventListener('click', e => {
    const nav = e.target.closest('[data-nav], [data-sidebar-nav]');
    if (!nav) return;
    const target = nav.dataset.nav || nav.dataset.sidebarNav;
    if (target === 'labs') {
      setLabsChrome(true);
      document.dispatchEvent(new Event('labsPageShown'));
    } else {
      setLabsChrome(false);
    }
  });
  ...
  window.onWalletConnected = function(addr) {
    ...
    // Only act if we're currently on the labs page
    const labsPage = document.getElementById('page-labs');
    if (!labsPage || labsPage.style.display === 'none') return;
