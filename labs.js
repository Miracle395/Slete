    ...
    try {
     ...
     // Pull position summary for vol/traders
      ...
      if (summaryRes.ok) {
        ...
        // Use oracle spot (already fetched above) as the dividing line
        const spotForSplit = priceData?.spot ? Number(priceData.spot) : null;

        if (all.length > 0) {
          // DeepBook positions have is_up: true/false directly on each record
          const hasIsUp = all.some(p => p.is_up !== undefined);

          let upVol = 0, downVol = 0;

          if (hasIsUp) {
            // Use the actual field — most reliable
            upVol   = all.filter(p => p.is_up === true).reduce((s, p)  => s + (Number(p.total_cost) || 0), 0) / 1e6;
            downVol = all.filter(p => p.is_up === false).reduce((s, p) => s + (Number(p.total_cost) || 0), 0) / 1e6;
          } else if (spotForSplit) {
            // Fallback: split by strike vs current spot
            upVol   = all.filter(p => Number(p.strike || 0) >= spotForSplit).reduce((s, p) => s + (Number(p.total_cost) || 0), 0) / 1e6;
            downVol = all.filter(p => Number(p.strike || 0) <  spotForSplit).reduce((s, p) => s + (Number(p.total_cost) || 0), 0) / 1e6;
          }

          if (upVol + downVol > 0) {
            state.upVol   = upVol;
            state.downVol = downVol;
          }

          // Debug — remove after confirming
          console.log('[Labs] positions sample:', JSON.stringify(all[0]));
          console.log('[Labs] upVol:', upVol, 'downVol:', downVol, 'hasIsUp:', hasIsUp);
        }
      }
