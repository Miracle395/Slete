...
async function _fetchWalletBalances(addr) {
  try {
    const cfg = _suiCfg();
    const rpcUrl = _suiCfg().rpcUrl;
    const apiKey = _suiCfg().tatumKey;
...
for (const coin of coins) {
    ...
    if (type === '0x2::sui::SUI') {
        suiBalance = amt / 1e9;
    } else if (type === DUSDC_TYPE) {
        dusdcBalance = amt / 1e6;
    ...
    window.STATE.dusdcBalance = dusdcBalance;
    window.STATE.balance      = dusdcBalance; // Slete uses DUSDC
}
    ...
    console.log('[Wallet] Balances - SUI: ' + suiBalance.toFixed(4) + ', USDC: ' + usdcBalance.toFixed(2) + ', DUSDC: ' + dusdcBalance.toFixed(2));
  } catch (err) {
...
const short = addr.slice(0, 6) + '...' + addr.slice(-4);
 ...
 document.querySelectorAll('.btn-connect, #walletBtn').forEach(b => {
      b.textContent = short;
